...
\centering
\begin{tikzpicture}[
    block/.style={rectangle, draw, fill=blue!10, text width=2.5cm, text centered, minimum height=1cm, rounded corners, font=\small},
    arrow/.style={->, thick},
    node distance=1cm
]
    % Pipeline stages
    \node[block] (unpack) {Unpack\\Fields};
    \node[block, right=of unpack] (align) {Exponent\\Alignment};
    \node[block, right=of align] (compute) {Mantissa\\Add/Sub};
    \node[block, right=of compute] (norm) {Normalize};
    \node[block, right=of norm] (pack) {Round \&\\Pack};

    % Arrows
    \draw[arrow] (unpack) -- (align);
    \draw[arrow] (align) -- (compute);
    \draw[arrow] (compute) -- (norm);
    \draw[arrow] (norm) -- (pack);

    % Labels
    \node[above=0.3cm of unpack] {UNPACK};
    \node[above=0.3cm of align] {ALIGN};
    \node[above=0.3cm of compute] {COMPUTE};
    \node[above=0.3cm of norm] {NORMALIZE};
    \node[above=0.3cm of pack] {PACK};

    % Helper modules below
    \node[block, fill=green!20, below=1cm of compute] (adder) {Gate-Level\\Adder};
    \node[block, fill=green!20, below=1cm of align] (shifter) {Barrel\\Shifter};
    \node[block, fill=green!20, below=1cm of norm] (lzc) {Leading\\Zero Counter};

    \draw[arrow, dashed] (adder) -- (compute);
    \draw[arrow, dashed] (shifter) -- (align);
    \draw[arrow, dashed] (lzc) -- (norm);

\end{tikzpicture}
\caption{Addition/Subtraction Unit Pipeline}
\end{figure}
...
\begin{tikzpicture}[scale=0.8]
    ...
    % State (using boxes)
    \draw[fill=gray!20] (1,2) rectangle (3,2.5);
    \node at (2,2.25) {\tiny UNPACK};
    \draw[fill=gray!20] (3,2) rectangle (5,2.5);
    \node at (4,2.25) {\tiny ALIGN};
    \draw[fill=gray!20] (5,2) rectangle (7,2.5);
    \node at (6,2.25) {\tiny COMPUTE};
    \draw[fill=gray!20] (7,2) rectangle (9,2.5);
    \node at (8,2.25) {\tiny NORM};
    \draw[fill=gray!20] (9,2) rectangle (11,2.5);
    \node at (10,2.25) {\tiny PACK};
    \node[left] at (0,2.25) {state};

    % Done signal
    \draw (0,1) -- (10,1) -- (10,1.5) -- (11,1.5) -- (11,1) -- (14,1);
...
\end{tikzpicture}
\caption{Typical Operation Timing (5 clock cycles per operation)}
\end{figure}
...
Both arithmetic units use a 5-stage pipeline:
\begin{enumerate}
    \item \textbf{UNPACK}: Extract sign, exponent, and mantissa fields
    \item \textbf{ALIGN/EXP\_ADD}: Align operands or add exponents
    \item \textbf{COMPUTE/MULTIPLY}: Perform mantissa operation
